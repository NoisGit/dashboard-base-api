...
class SystemService:
    ...
    async def get_system_counters(self) -> SystemCountersResponse:
        ...
        stmt_users = select(
            ...
            func.count().filter(User.role == UserRole.AGENT).label('count_agent'),
        ...
        return SystemCountersResponse(
            ...
            users_agents=res_users.count_agent,
    ...
    async def get_detail_admins(self) -> DetailAdminsResponse:
        ...
        Agent = aliased(User)
        AgentStaff = aliased(CompanyStaff)
        # Admin Details
        stmt = (
            select(
                ...
                func.coalesce(func.count(func.distinct(UserLocationAccess.id)), 0).label(
                    "entrances_count"),
                func.coalesce(func.count(func.distinct(Agent.id)), 0).label(
                    "agents_count")
            )
            ...
            .outerjoin(AgentStaff, Company.id == AgentStaff.company_id)
            .outerjoin(
                Agent,
                and_(
                    AgentStaff.user_id == Agent.id,
